...
//     // const [data] = JSON.parse(res.target.responseText); // for => func
...
const getCountryData = country => {
  fetch(`https://restcountries.eu/rest/v2/name/${country}`).then(
    ...
    return fetch(`https://restcountries.eu/rest/v2/alpha/${neighbour}`).then(response => {
      return response.json(response);
    });
  }).then(data => {
    if (data) renderCountry(data, true);
  }).catch(err => {
    console.error(err);
  }).finally(() => {
    ;
...
// setTimeout(() => whereAmI(52.508, 13.381), 1500);
// setTimeout(() => whereAmI(19.037, 72.873), 4500);
// setTimeout(() => whereAmI(-33.933, 18.474), 3000);

// console.log('Test start');
// setTimeout(() => console.log('0 sec timer'), 0);
// Promise.resolve('Resolved promise 1').then(res => console.log(res));
// Promise.resolve('Resolved promise 2').then(res => {
//   for (let i = 3000000000; i > 0; i--) { let j = i; }
//   console.log(res);
// });
...
// wait(3).then(() => console.log('Solved'));
